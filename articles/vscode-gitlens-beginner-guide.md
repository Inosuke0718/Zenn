...
![alt text](https://github.com/Inosuke0718/Zenn/blob/main/images/git/git-howtouse-flow.png?raw=true)

この記事では、大きく分けて **「基本編」** と **「実践編」** の2ステップで進めていきます。

### 🏠 基本編：自分のパソコンで完結！

まずは、日々のコードの変更を記録し、GitHubに公開するまでの基本の流れをマスターしましょう。

1.  **初期化** ：Gitを使う準備をします。
2.  **記録（Add & Commit）** ：変更に名前をつけて、自分のPCに保存します。
3.  **公開（Push）** ：保存した記録をGitHubにアップロードします。

### 🌿 実践編：チーム開発や安全な作業のために

基本が分かったら、次は「本番用のコードを壊さない」ための賢い使い方を学びます。

4.  **枝分かれ（Branch）** ：新しい作業用の場所を作ります。
5.  **合流（Merge）** ：完成した作業を、元の場所にドッキングさせます。

「難しそう……」と思うかもしれませんが、すべてボタン操作だけで完結します！
