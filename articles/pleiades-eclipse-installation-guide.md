...
## 🌐 6. Web 開発の腕試し：JSP を動かしてみよう！

通常の Java プログラムが動いたら、次は **Web 開発（サーバー側）** の動作確認もしてみましょう。
「JSP とかサーブレットって難しそう…」と感じるかもしれませんが、最小構成なら驚くほど簡単に動きます！

### ① プロジェクトの作成

まずは、Web アプリ専用のプロジェクトを作ります。

1.  **ファイル > 新規 > 動的 Web プロジェクト** を選択。
2.  プロジェクト名（例：`jsp-helloworld`）を入力。
3.  **ターゲット・ランタイム** に Tomcat が設定されていることを確認して「完了」をクリック！

### ② JSP ファイルの作成

次に、ブラウザに表示する画面（JSP ファイル）を作ります。

1.  作成したプロジェクト内の **webapp** フォルダ（古いバージョンでは `WebContent`）を右クリック。
2.  **新規 > JSP ファイル** を選択。
3.  ファイル名を `index.jsp` として作成します。
4.  開いたファイルの中身（`<body>`タグの中）に、Java のコードを 1 行だけ埋め込んでみましょう。

```html
<body>
    <h1>JSPテスト</h1>
    現在時刻は: <%= new java.util.Date() %> です！
</body>
```

> **💡 ワンポイント解説**
> `<%= ... %>` という記法は、Java の実行結果をそのまま画面に出力するための **JSP 特有の書き方** です。

### ③ 実行する

作成した `index.jsp` を右クリックして、 **実行 > サーバーで実行** を選択し、Tomcat を起動します。

ブラウザが立ち上がり、「JSP テスト」という文字と一緒に **現在の時刻** が表示されれば大成功です！🎉
Java クラス（Servlet）の作成や難しい設定などは一切不要で、これだけで Java がサーバー上で動いていることが確認できます。

---

## 🐛 7. デバッグの基本：動作を「見える化」する
...
## 🎨 8. ツールアイコンを見やすく！大きくする方法

高解像度のディスプレイ（4K など）をつかっていると、「Eclipse のアイコンが小さすぎて目が疲れる…」なんてこと、ありませんか？

実は、設定ファイルを少し書き換えるだけで、ツールバーのアイコンを自分好みのサイズに大きくできるんです。

### 🛠️ やるべき作業はとっても簡単！

Eclipse がインストールされているディレクトリ内にある `eclipse.ini` という設定ファイルを探し出し、テキストエディタで編集して以下の **3行** を最後に追記してください。
...
この例では `130` （130%）に設定していますが、お使いのディスプレイ解像度やサイズ、そしてあなたの **「眼の具合」** に合わせて自由に調節してみてください。

- **4K ディスプレイの方**: 派手めに大きく設定してみると、見違えるほど快適になりますよ！
- **注意点**: あまり大きくしすぎると、ツールバーが 2段になったり、エディタ部分が手狭になったりするので **要注意** です。
